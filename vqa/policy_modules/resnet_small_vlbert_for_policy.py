...
class PolicyResNetVLBERT(Module):
    def __init__(self, config):

        super(PolicyResNetVLBERT, self).__init__(config)

        self.enable_cnn_reg_loss = config.POLICY.ENABLE_CNN_REG_LOSS
        if not config.POLICY.BLIND:
            self.image_feature_extractor = FastRCNN(config,
                                                    average_pool=True,
                                                    final_dim=config.POLICY.IMAGE_FINAL_DIM,
                                                    enable_cnn_reg_loss=self.enable_cnn_reg_loss)
            if config.POLICY.VLBERT.object_word_embed_mode == 1:
                self.object_linguistic_embeddings = nn.Embedding(81, config.POLICY.VLBERT.hidden_size)
            elif config.POLICY.VLBERT.object_word_embed_mode == 2:
                self.object_linguistic_embeddings = nn.Embedding(1, config.POLICY.VLBERT.hidden_size)
            elif config.POLICY.VLBERT.object_word_embed_mode == 3:
                self.object_linguistic_embeddings = None
            else:
                raise NotImplementedError
        self.image_feature_bn_eval = config.POLICY.IMAGE_FROZEN_BN

        self.tokenizer = BertTokenizer.from_pretrained(config.POLICY.BERT_MODEL_NAME)

        language_pretrained_model_path = None
        if config.POLICY.BERT_PRETRAINED != '':
            language_pretrained_model_path = '{}-{:04d}.model'.format(config.POLICY.BERT_PRETRAINED,
                                                                      config.POLICY.BERT_PRETRAINED_EPOCH)
        elif os.path.isdir(config.POLICY.BERT_MODEL_NAME):
            weight_path = os.path.join(config.POLICY.BERT_MODEL_NAME, BERT_WEIGHTS_NAME)
        ...
        dim = config.POLICY.VLBERT.hidden_size
        if config.POLICY.CLASSIFIER_TYPE == "2fc":
            self.final_mlp = torch.nn.Sequential(
                torch.nn.Dropout(config.POLICY.CLASSIFIER_DROPOUT, inplace=False),
                torch.nn.Linear(dim, config.POLICY.CLASSIFIER_HIDDEN_SIZE),
                torch.nn.ReLU(inplace=True),
                torch.nn.Dropout(config.POLICY.CLASSIFIER_DROPOUT, inplace=False),
                torch.nn.Linear(config.POLICY.CLASSIFIER_HIDDEN_SIZE, config.POLICY.OUTPUT_SIZE),
            )
        elif config.POLICY.CLASSIFIER_TYPE == "1fc":
            self.final_mlp = torch.nn.Sequential(
                torch.nn.Dropout(config.POLICY.CLASSIFIER_DROPOUT, inplace=False),
    ...
    def train_forward(self,
        ...
        text_input_ids, text_token_type_ids, text_tags, text_mask, ans_pos = self.prepare_text_from_qa(question_ids,
                                                                                                       ...
                                                                                                       answer_tags,
                                                                                                       answer_mask)
        if self.config.POLICY.NO_GROUNDING:
        ...
        assert self.config.POLICY.VLBERT.object_word_embed_mode == 2
